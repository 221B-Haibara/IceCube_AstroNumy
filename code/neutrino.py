...
class NuDist:
    def __init__(self, ehefile="../Resources/EHE/EHE_effective.csv"):
        passSR = pd.read_csv(ehefile)
        bins = np.linspace(-1, 1, 180)
        m = plt.hist(passSR["cos(ImpLF_zen)"], weights=np.array(passSR["wE3"]) / 0.457845099495, bins=bins, normed=True)

        self.density_nocos = gaussian_kde(np.array(passSR["cos(ImpLF_zen)"]), weights=np.array(passSR["wE3"]))
        self.density_nocos.set_bandwidth(0.04)
        self.x_nocos = m[1]

        bins = np.linspace(0, 180, 180)
        m = plt.hist(np.rad2deg(np.arccos(passSR["cos(ImpLF_zen)"])), weights=np.array(passSR["wE3"]) / 0.457845099495,
                     bins=bins, normed=True)

        self.density = gaussian_kde(np.rad2deg(np.arccos(np.array(passSR["cos(ImpLF_zen)"]))),
                               weights=np.array(passSR["wE3"]))
